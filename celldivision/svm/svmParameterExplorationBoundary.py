...
for timeSize in [1, 2, 3, 5, 10, 15]:
    for voxelSize in range(5, 35, 5):
        for descriptorOrder in range(1, 5):
            featsTrain, labelsTrain = loadSetFromVideos(dirsTrain, datasetRoot, voxelSize, step, timeSize, descriptorOrder,True)
            featsTest, labelsTest = loadSetFromVideos(dirsTest, datasetRoot, voxelSize, step, timeSize, descriptorOrder,True)
            ...
            for CValue in [0.1, 1, 10, 100, 1000]:
                for kernelT in ['rbf', 'linear', 'poly']:
                    ...
                    with open(targetDir + str(numVideos) + 'VideosTrainBoundary.txt', 'a') as myfile:
                        myfile.write('\nNumVideos: ' + str(numVideos) + '\nTS: ' + str(timeSize) + '\nDO: ' + str(descriptorOrder) + '\nCV: ' + str(CValue) + '\nKT: ' + str(kernelT) + '\nVS: ' + str(voxelSize) + '\n' + classificationReport) 
                        
                    with open(targetDir + str(numVideos) + 'reportBoundary.txt', 'a') as myfile:
