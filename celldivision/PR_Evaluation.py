...
def Jaccard_Calculation(parametros):
	minDFactor = parametros[0]
	param2 = parametros[1]
	minRFactor = parametros[2]
	scores_TPFP = []
	for ii in range(0,frame_array.shape[0]):
		...
		start = time.time()
		init_circles = cv2.HoughCircles(im,cv2.HOUGH_GRADIENT,1,int(minDFactor*(radius[ii]/stage)),
	                            param1=50,param2=param2,minRadius=int(minRFactor*radius[ii]/stage),maxRadius=int(radius[ii]))
		end = time.time()
	...
	print('minDFactor:' + str(minDFactor) +' ,param2:' + str(param2) +' ,minRFactor:' +  str(minRFactor) + ' ,Mean Jaccard: ' + str(meanJaccard)+ '\n')
	with open('Hough/Complete/hough_paramsExploration_nms.csv','a') as f:
		f.write('minDFactor:' + str(minDFactor) +' ,param2:' + str(param2) +' ,minRFactor:' +  str(minRFactor) + ' ,Mean Jaccard: ' + str(meanJaccard)+ '\n')
	scores_FN = []
	scores_TPFP = []

processPool = mp.Pool(20)

minDFactor = np.array((0.5,0.7,0.9,1))
param2 = np.array((10,15,20,25,30,35))
minRFactor = np.array((1,1.5,1.7,1.9))
jacc = np.array((0.4,0.5,0.6,0.7))

parametros = []

for dist in minDFactor:
	for param in param2:
		for factor in minRFactor:
			parametros.append((dist,param,factor))
